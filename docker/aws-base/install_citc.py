...
parser.add_argument("--json", help="Provide a JSON file containing input "
                                   "parameters")
parser.add_argument("--region", help=f"Set the region in which the cluster will "
                                   f"be created (default {default_region})")
parser.add_argument("--key", help="Your public SSH key (either the key, file "
                                  "containing the key, or URL containing "
                                  "the key)")
parser.add_argument("--shape", help=f"The instance type used for the management "
...
def run_everything(args):
    ...
    while not region:
        region = input(f"Which region should the cluster run in "
                     f"[{default_region}]? ")
    ...
    while not login_shape:
        login_shape = input(f"What instance type should be used for the login node "
                            f"[{default_shape}]? ")
    ...
    if not has_completed("generate_keys"):
        run_command(f"ssh-keygen -t rsa -f "
                    f"{os.environ['HOME']}/.ssh/citc-aws "
                    f"-C provisioner -N \"\"")

    if not has_completed("init_terraform"):
        run_command("terraform init aws")
    ...
    if not has_completed("terraform_validate"):
        run_command("terraform validate aws")

    if not has_completed("terraform_plan"):
        run_command("terraform plan aws")

    if not has_completed("terraform_apply"):
        run_command("terraform apply -auto-approve aws")
    ...
    scp_options = "-o StrictHostKeyChecking=no -i ~/.ssh/citc-aws"
